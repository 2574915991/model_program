...
### 施密特网
1. 等积投影原理（兰伯特等积投影法）
   一个半径为1的单位球体，由式子 $x^2 + y^2 + (z-1)^2 = 1$定义，其中平面HAOE-H构成中央经线平面，位于XZ平面上且垂直于y轴，平面FAGE-F构成赤道平面，位于XOY平面上且垂直于z轴。一般地，这个球体的等积投影位于XY平面上，且相切于点O。在产状分析中我们一般使用“南半球”，即$z\leq 1$和单位球体的并集，以贴合“地质构造主要位于地下”这一理念。现在球面上有五个点：A、B、C、D、E，组成平面ABCDE-A，我们如何寻找这个平面的等积投影呢？根据这个球体的几何特性，z轴是这个球体的球极，那么球面上的圆弧OA、OB、OC、OD、OE可以各自位于球面五个大圆的一部分，而每个大圆都能构成一个平面，且这些平面都会穿过z轴。以O点为圆心，分别以OA、OB、OC、OD、OE为半径$r = 2Rsin(c / 2)$(默认单位球体R=1，c为直线与纵轴夹角)，分别在球面圆弧OA、OB、OC、OD、OE构成的大圆平面上画圆弧，与XOY平面相交形成A’、B’、C’、D’、E’五个点。如果我们在半圆弧ABCDE上取的点越多，我们会得到一条由A’B’C’D’E’构成的曲线。

   严谨地说，上例的A’B’C’D’E’其实是曲线ABCDE在该球体的南半球所构成平面的赤道方位等积投影，我们没有将E’A’两个点连起来。接下来的一个例子可以解释为什么我们最后没有收尾：按照上面例子给的方法寻找半圆平面AOE-A的赤道方位等积投影，A点落在A’上，O点落在自己，E点落在E’上，且三个投影点都位于x轴上。因此，该半圆平面的投影应该是A’OE’-A’；但是由于A’E’和E’A’是同一条线段，我们可以直接忽略收尾的线段E’A’。投影面上A’B’C’D’E’-A’由两条线段构成，即直线段E’A’和曲线段A’B’C’D’E’，但这两条线段由两个平面投影而来。

   面积保留是等积投影的一个最重要的性质，那么这个性质是哪两个面积相比较呢？还是以上图为例子，球面上的曲面ABCDE-OA由两条曲线构成：ABCDE和AOE。这两条曲线分别投影到XY平面上成为A’B’C’D’E’和A’OE’，那么平面A’B’C’D’E’-OA’的面积和球面上的曲面ABCDE-OA面积相等。同样地，半球面FAGE-O的赤道方位等积投影是圆F’A’G’E’，圆心位于O，且该半球面面积和圆F’A’G’E’的面积相等。
...
4. 施密特网与构造分析
   ...
   2. 圆锥形褶皱如下图所示，褶曲形面所构成的部分锥体叫 S 锥。锥体的旋转轴为锥轴（K），不位于形面上，在各个包含该轴的平面上，形面迹线与 K 间夹角（即半顶角 φ）相等。各个包含 K 的平面与形面交线称为脊线，只有相当于褶轴面的那个平面同形面的交线才被称为褶轴，顺褶轴方向，形面曲率向锥顶逐渐增大。在施密特网上，包含锥轴 K 的平面是不同倾斜方位和倾角的投影大圆，脊线投影点 \(d_1\)、\(d_2\) 等均分布于一个投影小圆上或附近，小圆半径角距为 \(\phi\)，显然，形面的\(\pi\)点必位于另一个圆锥上，这即是\(\pi S\)锥；可由\(d_1\)、\(d_2\) 等点分别取 \(90^\circ\) 的角距得出各点的连线而求出其投影。可以看出 S 锥与\(\pi S\)锥为共锥轴的两个锥体，但\(\pi S\)锥之半顶角为\(90^\circ\)-\(\phi\)。锥轴K是圆锥褶皱的构造轴线，投影平面是与轴K垂直的平面，也就是圆锥褶皱各切面法向量得到的直立小圆面（\(\pi S\)锥底面）
